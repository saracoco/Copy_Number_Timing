...
#orfeo
    ...
    self_name = as.character("example_pt_1")
    ...
    iter_dir <- paste0("/simulation_iteration_", 1)
    ...
    taus <- c(0.1,0.1,0.6,0.6,0.9)
    karyo <- c("2:0","2:1", "2:2", "2:1", "2:1")
    ...
    ggsave("./plots/simulation_data_all_segments.png", plot = simulation_plot, width = 12, height = 10,  device = png)
    ...
    # simulate the 4 events 3 of them happened before but a bit separated in time while another one happened clearly later (2 different time points)
    self_name = as.character("example_pt_2")
    new_dir = paste0("../",self_name) #relative path of the new created directory where to save the simulation results
    dir.create(new_dir)
    ...
    taus <- c(0.05,0.1,0.15,0.83)
    karyo <- c("2:0", "2:1", "2:1","2:0")
