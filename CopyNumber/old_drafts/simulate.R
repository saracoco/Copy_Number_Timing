...
for(i in 1:n_simulations){
  ...
  plotting_model_selection(results)
  ...
  Caption <- vector("list", (k_max))

  for (k in 1:k_max) {
    MSE <- readRDS(paste0("results/MSE_",k,".rds"))
    all_MSE <- c(all_MSE, MSE)
    Caption[[k]] <- paste0("MSE with ",k," clocks = ", MSE," ")
  }
  
  Caption <- paste(Caption, collapse = "   ")
  saveRDS(all_MSE, "results/all_MSE.rds")
