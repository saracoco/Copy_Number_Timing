...
#  }
 if (length(karyo) <= 2){
  ...
  delta_entropy_per_segments_norm = rep(0, k_max)
  delta_entropy_norm = rep(0, k_max)
  entropy_norm = rep(0, k_max)
 for (K in 1:k_max) {
    print(paste0("prepare_input_data, K = ",K))
    bnch<-bench::mark(input_data <- prepare_input_data(data, karyo, K, purity=purity))
    print(bnch)

      # inits_chain <- get_init(tau_single_inference, K)     # uncomment to run initialization with single segment model
    print(paste0("get_init_simpler, K = ",K))
    bnch <- bench::mark(inits_chain <- get_init_simpler(accepted_mutations, K, purity = purity))
    print(bnch)

    print(paste0("fit_variational, K = ",K))
    bnch<-bench::mark(res <- fit_variational(input_data, max_attempts=max_attempts, initialization = inits_chain, INIT = TRUE, tollerance = tollerance))
    print(bnch)
    ...
    print(paste0("compute_external_metric, K = ",K))
    bnch<-bench::mark(compute_external_metric(accepted_mutations, res, K))   
    print(bnch)
    ...
    # # delta_entropy[K] = mean(entropy_per_segment_matrix_norm[K,]) -mean(entropy_per_segment_matrix_norm[K-1,]) 

    #     entropy_norm[K] = entropy
    #   # entropy[K] = entropy_norm

    #   if(K==1){
    #  delta_entropy_norm[K] = entropy_norm[K] - 0
    #   }else{ 
    #  delta_entropy_norm[K] = entropy_norm[K] - entropy_norm[K-1]
    #   }


    #   if(K==1){
    #   delta_entropy_per_segments_norm[K] = mean(entropy_per_segment_matrix_norm[K,]) - 0
    #   }else{
    #   delta_entropy_per_segments_norm[K] = mean(entropy_per_segment_matrix_norm[K,]) - mean(entropy_per_segment_matrix_norm[K-1,])
    #   }
   ...
   if (k_max>2){
    ...
    ggsave(paste0("./plot_entropy_per_segment_mean.png"),width = (6 + (K*2)), height = (5), plot = plot_entropy)
  #   plot_entropy_norm <- plotting_delta_entropy(entropy_norm,K)
  #   ggsave(paste0("./plot_entropy_norm.png"),width = (2* (K*2)), height = (5), plot = plot_entropy_norm)
   }
  ...
  #     best_K <- 2
  #     }else{
  #     best_K <- which(delta_entropy[-1][-1] < 0)[1] + 2}
  #   }else {
  #     best_K <- 1
  #   }
