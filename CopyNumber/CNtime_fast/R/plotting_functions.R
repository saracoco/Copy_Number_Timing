...
plotting_entropy <- function(entropy_per_segment_matrix, entropy_per_segment_matrix_norm, K){
  ...
  entropy_per_segment_matrix_trimmed <- entropy_per_segment_matrix[-1, ]
    df <- data.frame(
      ...
      mean_per_row = rowMeans(entropy_per_segment_matrix_trimmed),  # Media delle righe
      sum_per_column = rowSums(entropy_per_segment_matrix_trimmed)  # Somma delle righe
    )

    entropy_plot_list[[1]] <- ggplot(df, aes(x = k, y = mean_per_row)) +
      geom_line() +
      geom_point() +
...
plotting_delta_entropy <- function(delta_entropy,K){

  delta_entropy_trimmed <- delta_entropy[-1]
    df <- data.frame(
      k = 2:length(delta_entropy),  # Valori di k che vanno da 2 a k_max
      delta_e = delta_entropy_trimmed  # Somma delle colonne
    )

    p <- ggplot(df, aes(x = k, y = delta_e)) +
      geom_line() +
      geom_point() +
      labs(x = "k", y = "Delta mean entropy over segments", title = paste0("Difference between K and K-1 of the mean of entropy across segment for k from 2 to k_max = ", K), 
      subtitle = "The entropy of the distribution of w is multiplied by the number of mutations per segment") +
      theme_bw() #theme_minimal()
